...
人物身份页以 QQ 号建卡，同一个 QQ 在群聊、私聊出现的昵称会成为同一张卡的别名。正式名字默认首次见到的昵称，代词默认 `TA`（不猜性别），类型按 user/assistant 初始化为人类/AI；这些字段可在面板手动或批量修改。合并误认人物时须明确指定目标 QQ，系统建立来源 QQ → 目标 QQ 映射，保留原始消息与参与者中的 QQ 证据。新事件提取和召回的参与者展示优先用正式名字；已经生成的旧 episode 正文不会自动重写。
